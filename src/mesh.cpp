 ...
 * 		mesh class stores geometry data for drwaing primitives in openGL.			*
 *      This code referred and modified codes from                                  *
...
// render the mesh
void Mesh::draw(GLenum DRAW_MODE, Shader &shader, const glm::mat4 &model, const glm::vec3 &color) const 
{    
    // draw mesh
...
Cube::Cube(float scale_) : scale(scale_) {
    setupMesh();
};
Cube::~Cube() {
...
void Cube::draw(GLenum DRAW_MODE, Shader &shader, const glm::mat4 &model, const glm::vec3 &color) const {

    //shader.use();

    glBindVertexArray(VAO);
    shader.setVec3("vColor", color);
	shader.setMat4("model", model);
    //glDrawElements(GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, 0);
	glDrawArrays(DRAW_MODE, 0, 3 * 2 * 6);
    glBindVertexArray(0);
}
...
void Cube::setupMesh(){
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(cubeVertices), 
		cubeVertices, GL_STATIC_DRAW);	// GL_DYNAMIC_DRAW, GL_STREAM_DRAW

	//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
	//glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), 
	//	indices, GL_STATIC_DRAW);	// GL_DYNAMIC_DRAW, GL_STREAM_DRAW

	// link vertex attributes
	// This function needs a careful look:
	// - vertex attributes position (later vertex shader refers by 'location = x')
	// - size of the vertex attribute (here its x, y, z)
	// - data type
	// - normalize data? (not too much to care about)
	// - the stride : size between next block (leaving 0 let OpenGL to figure it out)
	// - the offset : space between next block
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);	// position attr
	glEnableVertexAttribArray(0);	// position 0
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float)));	// normal attr
	glEnableVertexAttribArray(1);	// position 1
	//glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float)));	// color attr
	//glEnableVertexAttribArray(1);	// position 1
...
///////////////////////////////////////////////////////////////////////////////
void Icosphere::draw(GLenum DRAW_MODE, Shader &shader, const glm::mat4 &model, const glm::vec3 &color) const
{
    // draw mesh
    glBindVertexArray(VAO);
    shader.setVec3("vColor", color);
